...
function create(parent, nX, nY, nWidth, nHeight, bStartVisible)
    ...
    -- Support for CRT mode, forwarding to parent terminal if available
    function window.getCRT()
        if parent.getCRT then
            return parent.getCRT()
        end
        return false
    end

    -- CRT mode setter, consistent with the TermAPIMixin implementation
    -- Can be called with no arguments to toggle, or with a boolean to set explicitly
    function window.setCRT(enabled)
        if parent.setCRT then
            -- Forward to parent with same arguments
            return parent.setCRT(enabled)
        end
        return false
    end
